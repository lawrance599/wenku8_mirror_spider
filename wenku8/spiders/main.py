import scrapy
from wenku8.models import Book, engine, Chapter
...
class MainSpider(scrapy.Spider):
    ...
    def distribute_requests(self, response):
        ...
        update_stat = select(Book.id).where(
            and_(
                Book.last_updated
                <= date.today() - timedelta(self.UPDATE_DAYS_INTERVAL[1]),
                Book.last_updated
                >= date.today() - timedelta(self.UPDATE_DAYS_INTERVAL[0]),
                Book.status == "连载中",
            )
        )
        # 新书爬取
        ...
        with Session(engine) as session:
            ...
            update_urls = map(
                lambda x: f"https://www.wenku8.net/modules/article/packshow.php?id={x}&type=txt",
                update_ids,
            )
            update_tasks = list(
                map(
                    lambda x: Task(id=x[0], url=x[1], cb=self.parse_chapters),
                    zip(update_ids, update_urls),
                )
            )
            ...
            new_tasks: List[Task] = list(
                map(
                    lambda x: Task(id=x[0], url=x[1], cb=self.parse_main),
                    zip(new_ids, new_urls),
                )
            )
            ...
            for task in update_tasks + new_tasks:
                yield scrapy.Request(
                    url=task.url, callback=task.cb, meta={"id": task.id}
                )
    ...
    def parse_main(self, response):
        ...
        writer = response.css(
            "#content > div > table tr+ tr td:nth-child(2)::text"
        ).get()
        status = response.css(
            "#content > div > table tr+ tr td:nth-child(3)::text"
        ).get()
        last_updated = response.css(
            "#content > div > table tr+ tr td:nth-child(4)::text"
        ).get()
        words = response.css(
            "#content > div > table tr+ tr td:nth-child(5)::text"
        ).get()
        description = response.xpath(
            "//div[@id='content']/div/table[2]//td/span[6]/text()"
        ).get()
        ...
        if img_url is not None:
            yield scrapy.Request(
                url=img_url, callback=self.download_cover, meta={"id": id}
            )
        else:
            self.log(f"[new] <{id}> 未搜索到封面链接", 20)

        chapter_page_url = response.xpath(
            "//div[@id='content']//div[6]//div[1]/a/@href"
        ).get()
        if chapter_page_url is not None:
            yield scrapy.Request(
                url=chapter_page_url, callback=self.parse_chapters, meta={"id": id}
            )
        else:
    ...
    def parse_chapters(self, response):
        ...
        titles = list(
            map(
                lambda x: x.strip(),
                response.xpath("//td[@class='odd']/text()").getall(),
            )
        )
        ...
        def extract_cid(url):
            pattern = (
                r"https://dl.wenku8.com/packtxt.php\?aid=\d+&vid=(\d+)&charset=utf-8"
            )
        ...
        with Session(engine) as session:
            ...
            exists_chapter_ids = session.exec(
    ...
    def download_chapter(self, response):
        ...
        yield ChapterItem(
            id=id, book_id=book_id, serial=serial, title=title, content=text
        )
    ...
    def download_cover(self, response):
        ...
        yield CoverItem(id=id, content=content)
